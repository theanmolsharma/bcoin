...
class CoinSelector {
  ...
  constructor(tx, options) {
    ...
    this.useSelectBnB = false;
    this.useSelectByValue = false;
  ...
  fromOptions(options) {
    ...
    if(options.useSelectBnB) {
      assert(typeof options.useSelectBnB === 'boolean');
      this.useSelectBnB = options.useSelectBnB;
    }

    if(options.useSelectByValue) {
  ...
  async select(coins) {
    ...
    if (this.hardFee !== -1) {
      this.selectHard();
    } else if (this.useSelectBnB) {
      const filteredCoin = []
      for (const coin of this.coins) {
        if(this.isSpendable(coin))
          filteredCoin.push(coin);
      }
      this.coins = filteredCoin;
      await this.selectBnB();
    } else if(this.useSelectByValue) {
      const filteredCoin = []
      for (const coin of this.coins) {
        if(this.isSpendable(coin))
          filteredCoin.push(coin);
  ...
  async selectByValue() {
    ...
    this.index = this.coins.length - 1;
    let coin = this.coins[this.index];
    let value = Amount.value(coin.value);
    let spendingSize = await coin.estimateSpendingSize(this.getAccount);
    let effectiveValue = value - this.getFee(spendingSize);
    while (target >= effectiveValue) {
      ...
      value = Amount.value(coin.value);
      spendingSize = await coin.estimateSpendingSize(this.getAccount);
      effectiveValue = value - this.getFee(spendingSize);
    }
    ...
    if (target > 0) {
      ...
      const closestCoin = await this.findClosest(target);
      this.tx.addCoin(closestCoin);
      this.chosen.push(closestCoin);
    }

    // update fee
    for (const coin of this.chosen) {
      size += await coin.estimateSpendingSize(this.getAccount);
    }
    this.fee = this.getFee(size);
  }

  /**
   * Find closet coin to target
   * using Binary Search Algorithm
   * @param {Number} target
   * @returns {Coin}
   */

  async findClosest(target) {
    // Corner cases
    if (target <= Amount.value(this.coins[0].value))
      return this.coins[0];
    if (target >= Amount.value(this.coins[this.index].value))
    ...
    while (i < j) {
      ...
      let size = await coin.estimateSpendingSize(this.getAccount);
      let value = Amount.value(coin.value);
      const effectiveValue = value - this.getFee(size);
      ...
      if (target < effectiveValue) {
        if (mid > 0) {
          ...
          const prev = this.coins[mid - 1];
          size = await prev.estimateSpendingSize(this.getAccount);
          value = Amount.value(prev.value);
          const prevEffectiveValue = value - this.getFee(size);
          // if target is greater than previous
          // effective value, then return mid
          if (target > prevEffectiveValue)
            return coin;
        }
  ...
  async selectBnB() {
    // we don't need to add dummy output
    // for size estimation because
    // Branch and Bound won't produce change.
    ...
    let i = 0;
    for(const coin of this.coins) {
      const value = Amount.value(coin.value);
      const spendingSize = await coin.estimateSpendingSize(this.getAccount);
      const effectiveValue = value - this.getFee(spendingSize);
    ...
    while (tries-- > 0) {
      ...
      if (currValue + totalValue < target || currValue > target)
        backtrack = true;
      ...
      if (backtrack) {
        ...
        if (selected.length == 0 || utxo_pool_index - 1 == selected[selected.length - 1] || coin.value != coins[utxo_pool_index - 1].value) {
    ...
    if (tries == 0 || selected.length == 0)
      return

    for (const i of selected) {
      const index = coins[i].index;
