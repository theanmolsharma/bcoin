...
class CoinSelector {
  ...
  async init(coins) {
    ...
    for (const coin of coins) {
      if (this.isSpendable(coin)) {
        coin.effectiveValue = await this.getEffectiveValue(coin);
  ...
  async attemptSelection() {
    ...
    target += 100 * costOfSpendingChange;
    result = this.selectLowestLarger(target);
    if (result.length > 0)
      results.push([result, target]);
    result = this.selectSRD(target);
    ...
    // use waste metric to select best solution
    const bestWaste = CoinSelector.MAX_MONEY;
    result = [];
    for (const [selection, target] of results) {
      const currWaste = await this.getWaste(selection, costOfChange, target);
  ...
  async selectBnB(target, costOfChange) {
    ...
    let index = 0;
    for (const coin of this.coins)
      totalValue += coin.effectiveValue;
    ...
    while (tries-- > 0) {
      ...
      if (currValue + totalValue < target
        ...
        const currWaste = await this.getWaste(selected, 0, target);
        if (currWaste <= bestWaste) {
      ...
      if (backtrack) {
        ...
        while (--index > selected[selected.length - 1]) {
          totalValue += this.coins[index].effectiveValue;
        }

        // Remove last included UTXO from selected list.
        currValue -= this.coins[index].effectiveValue;
        selected.pop();
      } else {
        ...
        if  (selected.length === 0
          ...
          || coin.effectiveValue !== this.coins[index - 1].effectiveValue) {
          selected.push(index);
          currValue += this.coins[index].effectiveValue;
        }
      }
  ...
  selectLowestLarger(target) {
    ...
    this.index = 0;
    const selected = [];
    let effectiveValue = this.coins[this.index].effectiveValue;
    while (target >= effectiveValue) {
      ...
      effectiveValue = this.coins[this.index].effectiveValue;
    }
    ...
    if (target > 0 && this.index !== this.coins.length) {
      ...
      target -= this.coins[closestCoinIndex].effectiveValue;
  ...
  findLowestLarger(target) {
    ...
    if (target <= this.coins[j].effectiveValue)
      return j;
    ...
    while (i < j) {
      ...
      const effectiveValue = this.coins[mid].effectiveValue;
      ...
      if (target < effectiveValue) {
        if (mid < this.coins.length - 1) {
          ...
          const nextEffectiveValue = this.coins[mid + 1].effectiveValue;
  ...
  async getWaste(selected, costOfChange, target) {
    let waste = 0;
    ...
    for (const i of selected) {
      const coin = this.coins[i];
      selectedAmount += coin.effectiveValue;
      // Consider the cost of spending an input now vs in the future
      const spendingSize = await coin.estimateSpendingSize(this.getAccount);
      waste += (currentFeeRate - CoinSelector.LONG_TERM_FEERATE) * spendingSize;
    }
