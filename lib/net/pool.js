...
class Pool extends EventEmitter {
  ...
  async handleGetCFilters(peer, packet) {
    ...
    if (stopHeight - packet.startHeight + 1 >= common.MAX_CFILTERS)
      return;
  ...
  async handleGetCFHeaders(peer, packet) {
    ...
    let promises = [];
    for (let i = packet.startHeight; i <= stopHeight; i++) {
      promises.push(this.chain.getHash(i));
    }
    const blockHashes = await Promise.all(promises);
    promises = [];
    for (const blockHash of blockHashes) {
      promises.push(indexer.getFilterHash(blockHash));
    }
    const filterHashes = await Promise.all(promises);
...
class PoolOptions {
  ...
  fromOptions(options) {
    ...
    if (options.indexFilter != null) {
      assert(typeof options.indexFilter === 'boolean');
      this.indexFilter = options.indexFilter;
    }
    ...
    if (options.bip157 != null) {
      ...
      if (this.bip157)
        assert(this.indexFilter, 'Filter indexer is required for BIP 157');
    }
