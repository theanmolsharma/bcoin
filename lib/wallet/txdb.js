...
class TXDB {
  ...
  async saveCredit(b, credit, path) {
    ...
    const isSpendable = await this.isSpent(coin.hash, coin.index);
    if(isSpendable) {
      b.put(layout.u.encode(coin.value, coin.hash, coin.index), credit.toRaw());
      b.put(layout.U.encode(path.account, coin.hash, coin.index), null);
    }
